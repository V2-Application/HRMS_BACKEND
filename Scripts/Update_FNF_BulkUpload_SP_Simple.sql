-- Update FNF_BulkUpload stored procedure to work with current database structure
-- Drop existing procedure
IF EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'sp_FNF_BulkUpload')
    DROP PROCEDURE dbo.sp_FNF_BulkUpload;
GO

-- Create updated procedure (single parameter version)
...
BEGIN
    ...
    BEGIN TRY
        ...
        SELECT
            u.EmployeeId,
            'System',  -- Hardcoded for now since we don't have @CreatedBy parameter
            GETUTCDATE()
        FROM #Upload u;
