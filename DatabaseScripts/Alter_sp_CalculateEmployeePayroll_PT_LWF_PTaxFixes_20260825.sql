...
ALTER   PROCEDURE [dbo].[sp_CalculateEmployeePayroll_PT_LWF]               
    @EmployeeId INT,              
...
BEGIN              
 ...
 /* FALLBACK: StoreStateLinking is a legacy table that the Store-State mapping
    page does NOT maintain - that page writes tblLocation.StateId. A store
    mapped only through the UI (e.g. DM03 GWALIOR-HUB, StateId 269 = MADHYA
    PRADESH) resolved to NULL here, so PTax / LWF and every other state-driven
    deduction was silently skipped. Fall back to the location master.
    Added 2026-08-25. */
 IF @State IS NULL
...
WHEN NOT MATCHED THEN               
  ...
  /* PTax slab base = GROSS ACTUALLY PAYABLE for the month, not just the fixed
     components. Previously this read monthlyGrossCTC alone, which excludes the
     extra-day allowance / incentive / arrears / overtime that are paid in the
     month. An employee sitting on a slab ceiling (e.g. V49362: fixed 25,000.00
     + extra-day 403.23 = 25,403.23, Jharkhand nil band ends at 25,000) was
     therefore slabbed one band too low and had PTax of 0 instead of 100.
     Changed 2026-08-25. Backup: BACKUP_sp_CalculateEmployeePayroll_PT_LWF_Original_20260825.sql */
