/*
    Fix_PayrollMergeDuplicate_PTaxSkipped_20260826.sql
    --------------------------------------------------
    PROBLEM
      Aug-26 salary produced PTax = 0 for ALL 15,046 employees (Jul-26 had 2,178
      non-zero, Rs 324,179.83). Both write targets were empty -- tbl_Month_salary
      AND tblEmployeeDeductions -- so PTax was never COMPUTED, not overwritten.

    ROOT CAUSE (proven on prod 2026-08-26 by running one ecode in a rolled-back
    transaction and watching the row count change mid-run)

      1. sp_CalculateEmployeePayroll_PT_LWF_Dev contains an UNGUARDED
             INSERT into tbl_calculatePaybledays (...)
         with no DELETE and no NOT EXISTS. On a RE-RUN the employee already has
         a row for that ecode/month, so a SECOND row is created:

             BEFORE run   tbl_calculatePaybledays rows for V54678/Aug-26 = 1
             DURING run                                                  = 2

      2. Further down, MERGE dbo.tbl_Month_salary builds its source from
         tbl_calculatePaybledays joined on ecode+month. Two rows = two source
         rows for one target row, so the MERGE throws:
             "The MERGE statement attempted to UPDATE or DELETE the same row
              more than once."

      3. The TRY/CATCH at the end of the proc swallows it and only PRINTs
         'Error occurred in sp_CalculateEmployeePayroll'. THE SALARY RUN REPORTS
         SUCCESS.

      4. Everything after the MERGE never executes: the gross read, the PTax
         slab lookup, LWF, and every deduction write.

      That is exactly why the FIRST run of a month produces PTax and every
      RE-RUN produces zero. It is pre-existing behaviour, not caused by the
      2026-08-25 PTax/LWF edits -- those sit downstream of the failure point,
      which is why they appeared to have no effect.

    FIX
      Refresh the existing payable-days row instead of appending a new one.
      NOTHING IS DELETED OR TRUNCATED: an existing ecode/month row is UPDATEd in
      place, and only a genuinely new ecode/month gets an INSERT.

    SCOPE
      One proc: dbo.sp_CalculateEmployeePayroll_PT_LWF_Dev.
      No table, row, or index is dropped, deleted or truncated by this script.

    BACKUP
      The current live definition is saved alongside this file as
      BACKUP_sp_CalculateEmployeePayroll_PT_LWF_Dev_PreMergeFix_20260826.sql
      To roll back, run that file (change CREATE to ALTER).

    AFTER RUNNING THIS
      Execute the VERIFY block at the bottom (it rolls back, changes nothing).
      Expected: V54678 (Assam, gross 39,795) returns PTax = 208.00.
      Then re-run the Aug-26 payroll to correct PTax and Haryana LWF together.
*/
...
Select             
  ...
  + try_cast(ISNULL(Extra_day_allowence, 0) as decimal(18, 2))
  + try_cast(ISNULL(Incentive, 0)           as decimal(18, 2))
  + try_cast(ISNULL(Arrers, 0)              as decimal(18, 2))
  + try_cast(ISNULL(Overtime, 0)            as decimal(18, 2))
...
SELECT             
  @Lwf = MAX(            
    /* No state special-case (changed 2026-08-25). LWF for EVERY state is driven by
       the LWF policy page (LWFPolicyMaster): Employee is a FLAT monthly amount,
       capped by EmployeeMax when set, divided by Frequency. Haryana previously had
       a hardcoded branch that read the same column as a PERCENTAGE of earned gross,
       which the policy page cannot express. */
    CASE WHEN EmployeeMax IS NULL THEN ISNULL(Employee, 0)
         WHEN ISNULL(Employee, 0) > EmployeeMax THEN EmployeeMax
         ELSE ISNULL(Employee, 0) END / CASE WHEN Frequency = 'Monthly' THEN 1 WHEN Frequency = 'Half-yearly' THEN 6 WHEN Frequency = 'Yearly' THEN 12 ELSE 1 END
  ),             
  @LwfEmployeer = MAX(            
    CASE WHEN EmployeerMax IS NULL THEN ISNULL(Employeer, 0)
         WHEN ISNULL(Employeer, 0) > EmployeerMax THEN EmployeerMax
         ELSE ISNULL(Employeer, 0) END / CASE WHEN Frequency = 'Monthly' THEN 1 WHEN Frequency = 'Half-yearly' THEN 6 WHEN Frequency = 'Yearly' THEN 12 ELSE 1 END
  )             
FROM             
  LWFPolicyMaster             
WHERE             
...
PRINT '=== proc altered. Running VERIFY (rolls back, changes nothing) ===';
GO

/* ---------------------------------------------------------------------------
   VERIFY -- runs payroll for ONE ecode inside a transaction and rolls back.
   No data is kept. Expected: PTax_After = 208.00 for V54678.
   --------------------------------------------------------------------------- */
DECLARE @Ecode varchar(50) = 'V54678', @Month varchar(10) = 'Aug-26';

SELECT 'BEFORE' AS Stage,
       ISNULL(CAST(PTax AS varchar(20)), 'NULL') AS PTax,
       (SELECT COUNT(*) FROM tbl_calculatePaybledays
        WHERE Ecode = @Ecode AND [Month] = @Month) AS PaybleDaysRows
FROM tbl_Month_salary WHERE ecode = @Ecode AND [MONTH] = @Month;

BEGIN TRAN;

  EXEC dbo.prc_runecode_iterate_single_PT_LWF_Dev @Ecode = @Ecode, @Month = @Month;

  SELECT 'AFTER RUN (inside tran)' AS Stage,
         ISNULL(CAST(PTax AS varchar(20)), 'NULL') AS PTax,
         ISNULL(CAST(Lwf  AS varchar(20)), 'NULL') AS Lwf,
         (SELECT COUNT(*) FROM tbl_calculatePaybledays
          WHERE Ecode = @Ecode AND [Month] = @Month) AS PaybleDaysRows
  FROM tbl_Month_salary WHERE ecode = @Ecode AND [MONTH] = @Month;
...
SELECT 'AFTER ROLLBACK (unchanged)' AS Stage,
       ISNULL(CAST(PTax AS varchar(20)), 'NULL') AS PTax
FROM tbl_Month_salary WHERE ecode = @Ecode AND [MONTH] = @Month;
GO
