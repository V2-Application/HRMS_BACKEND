-- =============================================================================
-- Alter usp_GetEmployeeChangeLog (2026-07-07)
-- Surface the system IP (tblEmployee.UpdatedIp) on each change-log entry as ChangedIp,
-- parallel to ChangedBy/ChangedOn. UpdatedIp is EXCLUDED from the diffed columns so it
-- never shows up as its own "field changed" row. Definition-only; no data touched.
-- =============================================================================
...
BEGIN
    ...
    WHERE c.object_id = OBJECT_ID(N'HRMS.dbo.tblEmployee')
      AND c.name NOT IN ('ValidFrom','ValidTo','UpdatedOn','CreatedBy','CreatedOn','DeletedBy','DeletedOn','UpdatedIp');
...
SELECT
    ...
    OldValue = COALESCE(
        CASE a.ColumnName
            ...
            WHEN ''SubDepartmentId1'' THEN (SELECT TOP 1 x.SubDepartmentName FROM HRMS.dbo.tblSubDepartment x WITH (NOLOCK) WHERE x.SubDepartmentId = TRY_CONVERT(INT, a.OldValue))
            WHEN ''SubDepartmentId2'' THEN (SELECT TOP 1 x.SubDepartmentName FROM HRMS.dbo.tblSubDepartment x WITH (NOLOCK) WHERE x.SubDepartmentId = TRY_CONVERT(INT, a.OldValue))
            WHEN ''SubDepartmentId3'' THEN (SELECT TOP 1 x.SubDepartmentName FROM HRMS.dbo.tblSubDepartment x WITH (NOLOCK) WHERE x.SubDepartmentId = TRY_CONVERT(INT, a.OldValue))
    ...
    NewValue = COALESCE(
        CASE a.ColumnName
            ...
            WHEN ''SubDepartmentId1'' THEN (SELECT TOP 1 x.SubDepartmentName FROM HRMS.dbo.tblSubDepartment x WITH (NOLOCK) WHERE x.SubDepartmentId = TRY_CONVERT(INT, a.NewValue))
            WHEN ''SubDepartmentId2'' THEN (SELECT TOP 1 x.SubDepartmentName FROM HRMS.dbo.tblSubDepartment x WITH (NOLOCK) WHERE x.SubDepartmentId = TRY_CONVERT(INT, a.NewValue))
            WHEN ''SubDepartmentId3'' THEN (SELECT TOP 1 x.SubDepartmentName FROM HRMS.dbo.tblSubDepartment x WITH (NOLOCK) WHERE x.SubDepartmentId = TRY_CONVERT(INT, a.NewValue))
