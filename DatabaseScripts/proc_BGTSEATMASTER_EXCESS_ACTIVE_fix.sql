/*
    proc_BGTSEATMASTER — EXCESS-shows-Inactive fix (2026-07-23).

    Problem: The ACTIVE column is the SEAT's budget flag (BGTSEATMASTER.ACTIVE).
    For EXCESS rows (an active employee with NO budgeted seat / over-hire), the
    seat side of the FULL OUTER JOIN is NULL, so ACTIVE came back NULL and the
    export rendered it as "Inactive" — even though the employee is IsActive=1.

    Fix: one line in the Matched CTE — if there is no seat but there IS an
    employee (e.Ecode NOT NULL), treat the row as Active. Assigned/vacant seats
    keep the seat's own ACTIVE flag. No table or data change.
*/
...
Matched AS (
    SELECT COALESCE(e.STCode, s.LOC_CODE) AS STCode, COALESCE(e.DepartmentId, s.DEPT_SNO) AS DepartmentId,
        ...
        -- FIX: over-hire (no seat but an active employee) is Active, not NULL/Inactive
