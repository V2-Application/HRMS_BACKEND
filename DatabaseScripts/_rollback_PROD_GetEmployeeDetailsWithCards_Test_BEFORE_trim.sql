...
BEGIN
    ...
    /*
      OPTIMIZED: previously this proc built ALL ~55k employees (60 cols, experience CTEs, many joins,
      DISTINCT) into a TABLE VARIABLE and then paginated -> 2.5-3.5s per page load.
      Now it:
        1) builds a lightweight filtered key set (#Filtered: EmployeeId, IsActive, STCode) once,
        2) computes the count cards from it,
        3) slices the requested page of EmployeeIds (#PageEmp),
        4) enriches ONLY that page (experience CTE limited to the page) for the result set.
      Output columns/shape are identical to the original.
    */
