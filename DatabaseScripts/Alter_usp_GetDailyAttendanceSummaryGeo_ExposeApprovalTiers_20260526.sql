/*================================================================================
  dbo.usp_GetDailyAttendanceSummaryGeo — expose ManagerApprovalStatusId.

  Before: GeoAttendanceApproval JOINed but ga.ManagerApprovalStatusId was never
  projected → frontend always showed Manager Status = Pending even after a
  successful approve/reject (because record.managerApprovalStatusId came back
  null/missing and the JSX defaults null to Pending).
  After: ManagerApprovalStatusId is returned in result set #1.

  Minimal change — only the Manager Status column is fixed (per user request).
  No data touched. Read-side only. Idempotent (CREATE OR ALTER).
================================================================================*/
SET ANSI_NULLS ON;
...
PRINT '>> usp_GetDailyAttendanceSummaryGeo ALTERed: ManagerApprovalStatusId now returned';
GO
