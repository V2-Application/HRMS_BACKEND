...
BEGIN
    ...
    */
    ...
    WHERE e.IsActive = 0 AND e.ECode LIKE 'V%' AND NOT EXISTS (SELECT 1 FROM dbo.tblDesignation dx WITH (NOLOCK) WHERE dx.DesignationId = e.DesignationId AND dx.DesignationName LIKE '%NAPS%') AND YEAR(COALESCE((SELECT MAX(h.UpdatedOn) FROM dbo.tblEmployeeActiveInActiveHistories h WITH (NOLOCK) WHERE h.EmpId = CAST(e.EmployeeId AS NVARCHAR(50)) AND h.ActionPerformed='False'), e.UpdatedOn, e.DateOfLeft)) = 2026                              -- separated (from the master)
