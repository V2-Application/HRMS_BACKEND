CREATE OR ALTER PROCEDURE dbo.usp_LocationWiseGeofenceReport
...
BEGIN
    ...
    /*
      LOC-WISE TD/MTD GEO-FENCING GAP REPORT  (read-only)
      One row per location:
        ...
        TD         = geo-fence attendance days (materialized Status='GF') at the store for @AsOfDate (yesterday)
        MTD        = geo-fence attendance days for the current pay cycle through @AsOfDate
    ...
    */
    ...
    CREATE TABLE #Geo(LocationId INT NOT NULL PRIMARY KEY, TD INT NOT NULL, MTD INT NOT NULL);
    INSERT INTO #Geo(LocationId, TD, MTD)
    SELECT e.LocationId,
    ...
    LEFT JOIN #Geo g ON g.LocationId = l.LocationId
    ORDER BY l.STCode;
