...
SELECT             
  @Lwf = MAX(            
    /* HARYANA (changed 2026-08-25): flat EmployeeMax per month (Rs 35), regardless
       of days worked or earned gross. Previously 0.2% of the EARNED gross capped at
       35, so a part-month leaver such as V56713 (0.5 day, earned 245.50) paid only
       0.49. Other states are unaffected - they already take a flat Employee value. */
    CASE WHEN State = 'Haryana' THEN ISNULL(EmployeeMax, ISNULL(Employee, 0)) / CASE WHEN Frequency = 'Monthly' THEN 1 WHEN Frequency = 'Half-yearly' THEN 6 WHEN Frequency = 'Yearly' THEN 12 ELSE 1 END ELSE
...
12 ELSE 1 END END            
  ...
  @LwfEmployeer = MAX(            
    /* HARYANA employer side kept symmetric: flat EmployeerMax (Rs 70). */
    CASE WHEN State = 'Haryana' THEN ISNULL(EmployeerMax, ISNULL(Employeer, 0)) / CASE WHEN Frequency = 'Monthly' THEN 1 WHEN Frequency = 'Half-yearly' THEN 6 WHEN Frequency = 'Yearly' THEN 12 ELSE 1 END ELSE
