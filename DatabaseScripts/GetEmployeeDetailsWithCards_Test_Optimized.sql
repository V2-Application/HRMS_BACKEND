...
BEGIN
    ...
    /*
      OPTIMIZED + TRIM/TUNE:
        1) builds a lightweight filtered key set (#Filtered) once  -> used for the COUNT CARDS,
           which therefore stay computed over the FULL set (badges unchanged).
        2) computes the count cards from #Filtered.
        3) slices the EmployeeIds to ENRICH into #PageEmp. For @Mode='mainview' (the Employee
           Master active view) only ACTIVE + ABSCONDED (ResignationTypeId=10) employees are
           enriched, because the active tabs never display plain-inactive rows (the Inactive
           tab loads via its own separate paginated call). This avoids enriching ~44k rows
           that are thrown away client-side.
        4) enriches ONLY #PageEmp.
      TUNE: CreatedBy/UpdatedBy self-joins made SARGable (seek the PK instead of casting it);
            FORMAT() month-year replaced with set-based DATENAME/CONVERT (FORMAT is slow).
      Output columns/shape are identical to the original. No base-table data is modified.
    */
