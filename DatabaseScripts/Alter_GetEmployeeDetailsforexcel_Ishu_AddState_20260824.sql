/*
    Alter_GetEmployeeDetailsforexcel_Ishu_AddState_20260824.sql
    PROD: 192.168.151.28\hrms, database HRMS

    Adds a [State] column to the Employee Master reports.

    Both employee-master outputs read this one proc, so this single change covers
    both with no code deploy:
        GET employee/download-excel  (EmployeeController.DownloadEmployeeExcel -> LoadEmployeeExcelDataTableAsync)
        GET employee/employee-data   (EmployeeController.GetEmployeeData)
    Both stream every column the proc returns (DataTable.Load / reader loop), so
    the new column appears automatically.

    Source of the state value
    -------------------------
    dbo.StoreStateLinking ([ST-CD] -> State), matched on the employee's location
    STCode. That table is the only clean state master: 734 rows, 734 distinct
    store codes, no duplicates, 34 states.

    NOT tblLocation.StateId -> tblState: despite the name, tblState holds
    city/area names (ARA, BANKA, BARH, BGPR...), not states, and StateId is NULL
    for 172 of 796 locations.

    Coverage: 15,732 of 15,943 active employees resolve to a state. The 211 that
    do not are 68 employees with no location at all, 4 at DM03 GWALIOR-HUB (store
    missing from StoreStateLinking) and the rest at placeholder ST-CD-nnn stores.
    Those rows return NULL / blank in the column rather than failing.

    Column position: immediately after [Joined Location].
    Only two lines change; everything else is the proc exactly as captured from
    prod today (backup: BACKUP_GetEmployeeDetailsforexcel_Ishu_Original_20260824.sql).
*/

ALTER PROCEDURE [dbo].[GetEmployeeDetailsforexcel_Ishu]              
    @IsActive BIT = 1,              
    @AllEmployee BIT = 0,              
    @CompanyId INT = 0              
AS              
...
END AS [Attachment Link],    
    ...
    FROM tblEmployee e                  
        ...
        LEFT JOIN StoreStateLinking ssl ON UPPER(LTRIM(RTRIM(ssl.[ST-CD]))) = UPPER(LTRIM(RTRIM(l.STCode)))
