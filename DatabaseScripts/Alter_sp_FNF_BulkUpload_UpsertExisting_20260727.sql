-- =============================================================================
-- Alter dbo.sp_FNF_BulkUpload  (2026-07-27)
-- Completed-FNF uploader: full upsert on existing ecodes (like Employee Master).
--   * NEW ecodes (no FNF_Header)            -> INSERT full FNF  => Completed
--   * EXISTING ecodes (FNF_Header present)  -> UPDATE/upsert with sheet values
--        (processed OR already-completed alike; NO LONGER skipped)
--   * within-file duplicate ecodes / invalid ecodes -> reported, skipped
-- Blank sheet cells keep the existing DB value (COALESCE upsert), so a re-upload
-- only overwrites the columns actually provided.
-- @AlreadyDoneEcodes is retained for signature/back-compat but is now always NULL
-- because completed records are updated rather than skipped.
-- Additive/non-destructive: only this proc changes. No table/data drops.
-- Backup of prior definition: DatabaseScripts\Alter_sp_FNF_BulkUpload_ProcessedToCompleted_20260707.sql
-- =============================================================================
...
(
    ...
    @AlreadyDoneEcodes nvarchar(max) OUTPUT,   -- always NULL now (kept for back-compat)
    @ProcessedCount    int OUTPUT,             -- newly INSERTED FNFs
    @TotalRecords      int OUTPUT,             -- rows in the sheet
    @UpdatedCount      int = 0 OUTPUT          -- EXISTING FNFs upserted
)
AS
BEGIN
    SET NOCOUNT ON;
    ...
    BEGIN TRY
        ...
        --------------------------------------------------------------------
        -- 6. Find employees that already have an FNF_Header. ALL of them are
        --    upserted (processed OR already-completed) - nothing is skipped.
        --------------------------------------------------------------------
        ...
        INSERT INTO #Existing (EmployeeId, FNFId)
        SELECT h.EmployeeId, MAX(h.FNFId) AS FNFId
        FROM dbo.FNF_Header h WITH (NOLOCK)
        JOIN #Upload u ON u.EmployeeId = h.EmployeeId
        GROUP BY h.EmployeeId;

        --------------------------------------------------------------------
        -- 7a. UPSERT path: existing FNFs (processed OR completed) -> Completed
        --------------------------------------------------------------------
        -- FNF_Additions: update (COALESCE keeps existing where sheet blank)
        UPDATE a
        ...
        -- FNF_Additions: insert when the existing FNF has no additions row yet
        ...
        -- FNF_Payment: update -> sets a done-status (keeps/moves record to Completed)
        UPDATE p
        ...
        -- Count upserted (existing FNFs)
        SELECT @UpdatedCount = COUNT(*) FROM #Existing;

        -- Remove upserted rows so the INSERT (new) path only handles brand-new FNFs
