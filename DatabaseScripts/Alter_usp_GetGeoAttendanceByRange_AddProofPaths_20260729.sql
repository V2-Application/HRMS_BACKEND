-- =============================================================================
-- Alter dbo.usp_GetGeoAttendanceByRange -- add ProofPaths column   2026-07-29
-- -----------------------------------------------------------------------------
-- The SuperAdmin geofence export (api/EmpAttendance/geo/export) returns one row
-- per (Employee, PunchDate), but carried no reference to the punch proof files
-- that the on-screen Geofence Request table already exposes via `proofPath`.
--
-- Adds:
--   ProofPaths       NVARCHAR(MAX) -- trailing column; wwwroot-relative proof
--                                     paths for that punch day, DISTINCT,
--                                     pipe-separated (' | '), NULL when no
--                                     punch that day carried a proof.
--   SubDepartment1   \
--   SubDepartment2    > names off tblEmployee.SubDepartmentId1/2/3, placed
--   SubDepartment3   /  immediately after DepartmentName.
--
-- Only additive: every pre-existing column, filter and ORDER BY is unchanged.
-- NOTE the sub-department columns are inserted mid-projection, so any caller
-- reading this result set BY ORDINAL would shift. The API reads by name.
-- =============================================================================
